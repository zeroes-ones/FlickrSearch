...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    /// The Main application coordniator to avoid coupling to UIKit
    ...
    func application(
        ...
        appCoordinator = AppCoordinator(launchOptions: launchOptions)
        return true
    }
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    var sceneCoordinator: SceneCoordinator?
        ...
        guard let windowScene = scene as? UIWindowScene,
            let _ = (UIApplication.shared.delegate as? AppDelegate)?.appCoordinator else {
                assertionFailure("App delegate doesn't have app coordinator instance")
                return
        }
        let window = UIWindow(windowScene: windowScene)
        sceneCoordinator = SceneCoordinator(window: window, connectionOptions: connectionOptions)
    }
